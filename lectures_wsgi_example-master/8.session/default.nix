...
let
  ...
  beaker = pythonPackages.buildPythonPackage rec {
    name = "Beaker-1.8.0.tar.gz";

    src = pkgs.fetchurl {
      url = mirror://pypi/P/Paste/Beaker-1.8.0.tar.gz;
      sha256 = "1yy3nmkvynvnbmkr6wrmc0w32libv1sy16kmx9h79cc65nxna0bl";
    };
...
in rec {
  pyEnv = stdenv.mkDerivation {
    name = "py-jinja";
    buildInputs = [ stdenv php python3 mako whitenoise ]       ++
                  [ paginate beaker]                           ++
